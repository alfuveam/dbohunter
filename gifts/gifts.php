...
$categories = array(1 => 'Contas', 2 => 'Itens', 3 => 'Itens Em Containers', 4 => 'Enfeites');
					...
					if($productact == '' || !$productact) {
						if ($SHOP->points($_SESSION['name']) < $info['points']) {
							...
							if($info['type'] == '4') 
								...
								echo 'Ou outro jogador:';
							echo '</td>
							<td><input type="text" name="other_char" size="10"/></td>
							</tr>
							<tr>
							<td></td>
							<td><button type="submit" name="submit" value="true" class="ide_button">Comprar Produto</td>
							...
							if($send == true) {
								...
								elseif ($info['type'] == '2') {
									...
									if ($SHOP->isOnline($destination) == 1) {
										$errors .= 'Desculpe, mas o jogador <b>'.$destination.'</b> está Online. Deslogue para receber a compra.<br>';
										$finish = false;
									}
								...
								elseif ($info['type'] == '3') {
									if ($SHOP->isOnline($destination) == 1) {
										$errors .= 'Desculpe, mas o jogador <b>'.$destination.'</b> está Online agora. Deslogue para receber sua compra.<br>';
										$finish = false;
									}
									else {
										if ($SHOP->isBanned($destination)->fetch()) {
											...
											$errors .= 'Desculpe, mas a <b>'.$destination.'</b>\'s Account não está banido no momento.<br>';
											$finish = false;
										}
									}
								}
...
echo '<center>Você não tem pontos o suficiente!</center>';
		...
		elseif ($action == 'history') {
			if ($SQL->query('SELECT * FROM shop_history WHERE session = "'.$_SESSION['name'].'" ORDER BY date DESC LIMIT 30')->fetch()) {
				...
				alert('Você não fez quaisquer transações.');
			}
		}
		...
		elseif ($action == 'admin') {
			if($ide->isAdmin()) {
				...
				elseif ($manage == 'add') {
					...
					$types[3] = array(6 => 'Bag Com Itens', 7 => 'Backpack Com Itens');
					$types[4] = array(8 => 'Item');
						...
						if(isset($destination)) {
							...
							if ($_POST['submit'] == 'remove') {
								if ($SHOP->points($account['name']) == '0' || $SHOP->points($account['name']) < '0')
									$errors .= "O personagem <b>".$player['name']."</b> não tem pontos premium.<br>";
							}
						}
						if ($errors == '') {
							$player = $SQL->query('SELECT name FROM players WHERE id = "'.$destination.'"')->fetch();
